...
class ControllerExtensionPaymentMokaPayment extends Controller {
    ...
    private $order_prefix = "opencart23X_";
    ...
    public function index() {
        ...
        if (isset($record['result']) AND $record['result']) {
			...
			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('moka_payment_order_status_id'), $message, false);
      ...
      $total_cart = round($order_info['total'] * $order_info['currency_value'], 2);
        $moka_rates = MokaConfig::calculatePrices($total_cart, $this->config->get('moka_payment_rates'));

        $data['installments_mode'] = $this->config->get('moka_payment_installement');
        ...
        $data['action'] = $this->url->link('extension/payment/moka_payment', '', 'SSL');
            ...
            if (VERSION >= '2.2.0.0'){
                    $template_url = 'extension/payment/moka_payment.tpl';
                } else {
                    $template_url = 'default/template/extension/payment/moka_payment.tpl';
                }
