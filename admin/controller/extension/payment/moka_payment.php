...
class ControllerExtensionPaymentMokaPayment extends Controller {
    ...
    private $module_version = "2.3.0.0";
    ...
    public function index() {
        ...
        if ($this->config->get('moka_payment_rates') == NULL) {
	
            $this->config->set('moka_payment_rates', MokaConfig::setRatesDefault());
        ...
        if (isset($this->request->get['update_error'])) {
            ...
            if (!empty($versionCheck['version_status']) AND $versionCheck['version_status'] == '1') {
                ...
                $data['version_update_link'] = $this->url->link('extension/payment/moka_payment/update', 'token=' . $this->session->data['token'] . "&version=$version_updatable", true);
            }
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/moka_payment', 'token=' . $this->session->data['token'] , 'SSL'),
			
            'separator' => ' :: '
        );

        $data['action'] = $this->url->link('extension/payment/moka_payment', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $data['moka_rates_table'] = MokaConfig::createRatesUpdateForm($this->config->get('moka_payment_rates'));
        $this->load->model('localisation/order_status');
        if ($data['moka_payment_order_status_id'] == '') {
            $data['moka_payment_order_status_id'] = $this->config->get('config_order_status_id');
        }
        ...
        $this->response->setOutput($this->load->view('extension/payment/moka_payment.tpl', $data));
    ...
    public function update() {
      ...
      $updated = $this->model_extension_payment_moka_payment->update($version_updatable);
        if ($updated == 1) {
			$this->load->model('setting/setting');
			$moka_payment_dealercode=$this->config->get('moka_payment_dealercode');
			$moka_payment_username=$this->config->get('moka_payment_username');
			$moka_payment_password=$this->config->get('moka_payment_password');	
			$moka_payment_moka_3d_mode=$this->config->get('moka_payment_moka_3d_mode');	
			$moka_payment_status=$this->config->get('moka_payment_status');	
			$moka_payment_order_status_id=$this->config->get('moka_payment_order_status_id');
			$moka_payment_sort_order=$this->config->get('moka_payment_sort_order');	
			$moka_payment_installement=$this->config->get('moka_payment_installement');	
			$moka_payment_test_mode=$this->config->get('moka_payment_test_mode');	
			$moka_payment_rates=$this->config->get('moka_payment_rates');	
			$moka_payment_cancel_order_status_id=$this->config->get('moka_payment_cancel_order_status_id');	
			...
			$this->config->set('moka_payment_dealercode', $moka_payment_dealercode);
			$this->config->set('moka_payment_username', $moka_payment_username);
			$this->config->set('moka_payment_password', $moka_payment_password);
			$this->config->set('moka_payment_moka_3d_mode', $moka_payment_moka_3d_mode);
			$this->config->set('moka_payment_status', $moka_payment_status);
			$this->config->set('moka_payment_order_status_id', $moka_payment_order_status_id);
			$this->config->set('moka_payment_sort_order', $moka_payment_sort_order);
			$this->config->set('moka_payment_installement', $moka_payment_installement);
			$this->config->set('moka_payment_test_mode', $moka_payment_test_mode);
			$this->config->set('moka_payment_rates', $moka_payment_rates);
			$this->config->set('moka_payment_cancel_order_status_id', $moka_payment_cancel_order_status_id);
            ...
            $this->response->redirect($this->url->link('extension/payment/moka_payment', 'token=' . $this->session->data['token'] . "&update_error=$updated", true));
			}
    }
	...
	$moka_payment_test_mode = $this->config->get('moka_payment_test_mode');
	  ...
	  $url = 'https://service.moka.com/PaymentDealer/GetDealerPaymentTrxDetailList';
       ...
       return $this->load->view('extension/payment/moka_payment_order.tpl', $data);
    }
